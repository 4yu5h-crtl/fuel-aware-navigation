...
const FUEL_CONSUMPTION_RATE = 0.08; // Liters per kilometer
    ...
    if (result !== null && status === 'OK') {
      ...
      const options: RouteOption[] = result.routes.map(route => {
        ...
        return {
          ...
          duration,
          fuelRequired,
          isEfficient: fuelRequired <= fuelLevel
        };
      });

      // Sort routes by efficiency (fuel consumption) and feasibility
      options.sort((a, b) => {
        // Prioritize routes that are feasible with current fuel
        if (a.isEfficient && !b.isEfficient) return -1;
        if (!a.isEfficient && b.isEfficient) return 1;
        // Then sort by fuel consumption
